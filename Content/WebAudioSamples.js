...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,Site,AudioHolder,Option,AudioVisualizer,Uint8Array,List,Html,Client,Attr,Tags,Seq,Number,Operators,EventsPervasives,jQuery,Samples,T,window,ChannelSplitter,Set,TimeDomain,Client1,Builder,Operators1,Sample,document,Unchecked;
 Runtime.Define(Global,{
  Site:{
   AudioHolder:{
    ...
    {
     return Option.iter(function(e)
     {
      return e.stop();
     },AudioHolder.currentPlaying());
    },
    currentPlaying:Runtime.Field(function()
    {
     return{
      $:0
     };
   ...
   AudioVisualizer:{
    Analyser:function()
    {
     ...
     Global.sourceNode=javascriptNode;
     javascriptNode.connect(AudioVisualizer.context().destination);
     javascriptNode.onaudioprocess=function()
     {
      var array,ctx,gradient;
      array=new Uint8Array(analyser.frequencyBinCount<<0);
      analyser.getByteFrequencyData(array);
      ctx=AudioVisualizer.Canvas().Dom.getContext("2d");
     ...
     return Option.iter(function(e)
     {
      ...
      e.connect(AudioVisualizer.filter());
      e.buffer=(AudioVisualizer.buffer())[0];
      e.loop=true;
      return e.start();
     },AudioVisualizer.sourceNode());
    },
    ButtonEvent:function(biq)
    {
     AudioVisualizer.filter().type=biq;
    },
    Canvas:Runtime.Field(function()
    {
     var arg10;
     arg10=List.ofArray([Attr.Attr().NewAttr("width","1000"),Attr.Attr().NewAttr("height","325")]);
     return Tags.Tags().NewTag("canvas",arg10);
    }),
    DrawSpectrum:function(ctx,array)
    {
     return Seq.iteri(function(i)
     {
      return function(a)
      {
       return ctx.fillRect(Number(i*5),325-Number(a),3,325);
      };
     },AudioVisualizer.ToList(array));
    },
    Filters:Runtime.Field(function()
    {
     return List.ofArray(["allpass","bandpass","highpass","highshelf","lowpass","lowshelf","notch","peaking"]);
    }),
    GenRadioButton:function(filter)
    {
     var arg10,x,arg00,arg101,x1;
     arg10=List.ofArray([Attr.Attr().NewAttr("type","radio"),Attr.Attr().NewAttr("name","filters")]);
     x=Tags.Tags().NewTag("input",arg10);
     arg00=function(_arg10_)
     {
      return function(_arg20_)
      {
       return AudioVisualizer.ButtonEvent(filter,_arg10_,_arg20_);
      };
     };
     EventsPervasives.Events().OnClick(arg00,x);
     arg101=List.ofArray([x]);
     x1=Global.String(filter);
     return Operators.add(Tags.Tags().NewTag("div",arg101),List.ofArray([Tags.Tags().text(x1)]));
    },
    ...
    Main:function(elem)
    {
     ...
     ignition=function(buff)
     {
      return AudioVisualizer.context().decodeAudioData(buff,function(bff)
      {
       var arg10;
       (AudioVisualizer.buffer())[0]=bff;
       AudioVisualizer.Analyser();
       jQuery(AudioVisualizer.RadioGroup().get_Item(0).Dom).children().first().prop("checked","true");
       arg10=List.ofArray([Attr.Attr().NewAttr("style","display: inline-block")]);
       jQuery(elem).append(Operators.add(Tags.Tags().NewTag("div",arg10),AudioVisualizer.RadioGroup()).Dom);
       return;
      });
     };
     jQuery(elem).append(AudioVisualizer.Canvas().Dom);
     return AudioVisualizer.LoadSound("diesirae.mp3",ignition);
    },
    MkGradient:function(gradient)
    {
     gradient.addColorStop(1,"#000000");
     gradient.addColorStop(0.75,"#ff0000");
     gradient.addColorStop(0.25,"#ffff00");
     return gradient.addColorStop(0,"#ffffff");
    },
    RadioGroup:Runtime.Field(function()
    {
     return List.map(function(filter)
     {
      return AudioVisualizer.GenRadioButton(filter);
     },AudioVisualizer.Filters());
    }),
    Sample:Runtime.Field(function()
    {
     return Samples.Build().Id("AudioVisualizer").FileName("AudioVisualizer.fs").Keywords(List.ofArray(["webaudio","visualizer","frequency","spectrum"])).Render(function(elem)
     {
      return AudioVisualizer.Main(elem);
     }).Create();
    }),
    Stop:function()
    {
    ...
    {
     return AudioVisualizer.context().createBiquadFilter();
    }),
    sourceNode:Runtime.Field(function()
    {
     return{
      $:0
     };
   ...
   ChannelSplitter:{
    Analyser:function()
    {
     var analyser1,analyser2,gainNode,javascriptNode,splitter,_;
     ...
     javascriptNode.onaudioprocess=function()
     {
      var array,mapping,list,x,average1,array2,mapping1,list1,x1,average2,ctx,gradient;
      array=new Uint8Array(analyser1.frequencyBinCount<<0);
      analyser1.getByteFrequencyData(array);
      mapping=function(value)
      {
       return Number(value);
      };
      list=ChannelSplitter.ToList(array);
      x=List.map(mapping,list);
      average1=Seq.average(x);
      array2=new Uint8Array(analyser2.frequencyBinCount<<0);
      analyser2.getByteFrequencyData(array2);
      mapping1=function(value)
      {
       return Number(value);
      };
      list1=ChannelSplitter.ToList(array2);
      x1=List.map(mapping1,list1);
      average2=Seq.average(x1);
      ctx=ChannelSplitter.Canvas().Dom.getContext("2d");
     ...
     return Option.iter(function(e)
     {
      ...
      e.connect(gainNode);
      e.loop=true;
      e.buffer=(ChannelSplitter.buffer())[0];
      return e.start();
     },ChannelSplitter.sourceNode());
    },
    Canvas:Runtime.Field(function()
    {
     var arg10;
     arg10=List.ofArray([Attr.Attr().NewAttr("width","60"),Attr.Attr().NewAttr("height","130"),Attr.Attr().NewAttr("style","display: block;")]);
     return Tags.Tags().NewTag("canvas",arg10);
    }),
    LoadSound:function($url,$callback)
    {
    ...
    Main:function(elem)
    {
     var arg10,maind,ignition;
     ChannelSplitter.Stop();
     AudioHolder.StopCurrent();
     jQuery(ChannelSplitter.VolumeControl().Dom).on("input",function()
     {
      var fraction;
      fraction=jQuery(ChannelSplitter.VolumeControl().Dom).val()/jQuery(ChannelSplitter.VolumeControl().Dom).prop("max");
      (ChannelSplitter.volume())[0]=fraction*fraction;
      return;
     });
     arg10=List.ofArray([ChannelSplitter.Canvas(),ChannelSplitter.VolumeControl()]);
     maind=Tags.Tags().NewTag("div",arg10);
     ignition=function(buff)
     {
      return ChannelSplitter.context().decodeAudioData(buff,function(bff)
      {
       (ChannelSplitter.buffer())[0]=bff;
       return ChannelSplitter.Analyser();
      });
     };
     jQuery(elem).append(maind.Dom);
     return ChannelSplitter.LoadSound("diesirae.mp3",ignition);
    },
    MkGradient:function(gradient)
    {
     gradient.addColorStop(1,"#000000");
    ...
    Sample:Runtime.Field(function()
    {
     return Samples.Build().Id("ChannelSplitter").FileName("ChannelSplitter.fs").Keywords(List.ofArray(["webaudio","vizualiser","channels","volume control"])).Render(function(elem)
     {
      return ChannelSplitter.Main(elem);
     }).Create();
    }),
    Stop:function()
    {
    ...
    VolumeControl:Runtime.Field(function()
    {
     var arg10;
     arg10=List.ofArray([Attr.Attr().NewAttr("type","range"),Attr.Attr().NewAttr("min","1"),Attr.Attr().NewAttr("max","100"),Attr.Attr().NewAttr("value","100"),Attr.Attr().NewAttr("style","width: 60px")]);
   ...
   Client:{
    ...
    {
     var _op_BangPlus_8;
     _op_BangPlus_8=function(x)
     {
      return Set.Singleton(x);
     };
     return Set.Create(List.ofArray([_op_BangPlus_8(AudioVisualizer.Sample()),_op_BangPlus_8(ChannelSplitter.Sample()),_op_BangPlus_8(TimeDomain.Sample())]));
    }),
    Main:Runtime.Field(function()
    {
     return Client1.All().Show();
    })
   },
   Samples:{
    Build:function()
    ...
    Builder:Runtime.Class({
     Create:function()
     {
      ...
      title=Operators1.DefaultArg(Samples.op_PlusPlus(this.BTitle,this.BId),"Sample");
    ...
    Sample:Runtime.Class({
     Show:function()
     {
      var sMain,sSide,url,arg10,arg101,x,s=this,arg102;
      sMain=document.getElementById("sample-main");
      sSide=document.getElementById("sample-side");
      Samples.Clear(sMain);
      Samples.Clear(sSide);
      this.Render.call(null,sMain);
      url="http://github.com/intellifactory/websharper.webaudio/blob/master/Site/"+this.FileName;
      arg101=Runtime.New(T,{
       $:0
      });
      x=Tags.Tags().NewTag("div",arg101);
      ...
      arg102=List.ofArray([Attr.Attr().NewAttr("class","btn btn-primary btn-lg"),Attr.Attr().NewAttr("href",url)]);
      arg10=List.ofArray([x,Operators.add(Tags.Tags().NewTag("a",arg102),List.ofArray([Tags.Tags().text("Source")]))]);
      return Tags.Tags().NewTag("div",arg10).AppendTo("sample-side");
     }
    }),
    Set:Runtime.Class({
     Show:function()
     {
      ...
      return jQuery(function()
      {
       ...
       arg10=List.ofArray([Attr.Attr().NewAttr("class","nav nav-pills")]);
       navs=Operators.add(Tags.Tags().NewTag("ul",arg10),List.mapi(function(i)
       {
        return function(s1)
        {
         var arg101,arg102,x,x1,x2,arg00;
         arg102=List.ofArray([Attr.Attr().NewAttr("href","#")]);
         x=s1.Title;
         arg101=List.ofArray([Operators.add(Tags.Tags().NewTag("a",arg102),List.ofArray([Tags.Tags().text(x)]))]);
         x1=Tags.Tags().NewTag("li",arg101);
         Operators.OnAfterRender(function(self)
         {
    ...
    req:function(name,f)
    {
     return f.$==1?f.$0:Operators1.FailWith("Required property not set: "+name);
    }
   },
   TimeDomain:{
    Analyser:function()
    {
     ...
     javascriptNode.onaudioprocess=function()
     {
      ...
      return TimeDomain.DrawTimeDomain(TimeDomain.CanvasEl().Dom.getContext("2d"),array);
     };
     ...
     {
      AudioHolder.SetCurrent(e);
      e.connect(TimeDomain.filter());
      e.buffer=(TimeDomain.buffer())[0];
      e.loop=true;
      return e.start();
     },TimeDomain.sourceNode());
    },
    ButtonEvent:function(biq)
    {
     TimeDomain.filter().type=biq;
    },
    CanvasEl:Runtime.Field(function()
    {
     var arg10;
     arg10=List.ofArray([Attr.Attr().NewAttr("width","512"),Attr.Attr().NewAttr("height","256"),Attr.Attr().NewAttr("style","background-color: black")]);
     return Tags.Tags().NewTag("canvas",arg10);
    }),
    DrawTimeDomain:function(ctx,array)
    {
     var c,width,height;
     c=jQuery(TimeDomain.CanvasEl().Dom);
     ...
     return Seq.iteri(function(i)
     {
      ...
      {
       var y;
       y=height-height*(Number(a)/256)-1;
       ctx.fillStyle="#ffffff";
       return ctx.fillRect(Number(i),y,1,1);
      };
    ...
    GenRadioButton:function(filter)
    {
     ...
     arg00=function(_arg10_)
     {
      return function(_arg20_)
      {
       return TimeDomain.ButtonEvent(filter,_arg10_,_arg20_);
      };
     };
     EventsPervasives.Events().OnClick(arg00,x);
     arg101=List.ofArray([x]);
    ...
    Main:function(elem)
    {
     ...
     ignition=function(buff)
     {
      return TimeDomain.context().decodeAudioData(buff,function(bff)
      {
       var arg10;
       (TimeDomain.buffer())[0]=bff;
       TimeDomain.Analyser();
       jQuery(TimeDomain.RadioGroup().get_Item(0).Dom).children().first().prop("checked","true");
       arg10=List.ofArray([Attr.Attr().NewAttr("style","display: inline-block")]);
       jQuery(elem).append(Operators.add(Tags.Tags().NewTag("div",arg10),TimeDomain.RadioGroup()).Dom);
       return;
      });
     };
     jQuery(elem).append(TimeDomain.CanvasEl().Dom);
     return TimeDomain.LoadSound("diesirae.mp3",ignition);
    },
    RadioGroup:Runtime.Field(function()
    {
     return List.map(function(filter)
     {
      return TimeDomain.GenRadioButton(filter);
     },TimeDomain.Filters());
    }),
    Sample:Runtime.Field(function()
    {
     return Samples.Build().Id("TimeDomainVisualizer").FileName("TimeDomain.fs").Keywords(List.ofArray(["webaudio","timedomain","visualizer","audio"])).Render(function(elem)
     {
      return TimeDomain.Main(elem);
     }).Create();
    }),
    Stop:function()
    {
    ...
    filter:Runtime.Field(function()
    {
     return TimeDomain.context().createBiquadFilter();
    }),
    sourceNode:Runtime.Field(function()
    {
     return{
 ...
 Runtime.OnInit(function()
 {
  ...
  AudioHolder=Runtime.Safe(Site.AudioHolder);
  Option=Runtime.Safe(Global.WebSharper.Option);
  AudioVisualizer=Runtime.Safe(Site.AudioVisualizer);
  Uint8Array=Runtime.Safe(Global.Uint8Array);
  List=Runtime.Safe(Global.WebSharper.List);
  Html=Runtime.Safe(Global.WebSharper.Html);
  Client=Runtime.Safe(Html.Client);
  Attr=Runtime.Safe(Client.Attr);
  ...
  T=Runtime.Safe(List.T);
  window=Runtime.Safe(Global.window);
  ChannelSplitter=Runtime.Safe(Site.ChannelSplitter);
  Set=Runtime.Safe(Samples.Set);
  TimeDomain=Runtime.Safe(Site.TimeDomain);
  Client1=Runtime.Safe(Site.Client);
 ...
 Runtime.OnLoad(function()
 {
  ...
  TimeDomain.buffer();
  TimeDomain.Sample();
  TimeDomain.RadioGroup();
  TimeDomain.Filters();
  TimeDomain.CanvasEl();
  Client1.Main();
  Client1.All();
  ChannelSplitter.volume();
  ChannelSplitter.sourceNode();
  ChannelSplitter.context();
  ChannelSplitter.buffer();
  ChannelSplitter.VolumeControl();
  ChannelSplitter.Sample();
  ChannelSplitter.Canvas();
  AudioVisualizer.sourceNode();
  AudioVisualizer.filter();
  AudioVisualizer.context();
  AudioVisualizer.buffer();
  AudioVisualizer.Sample();
  AudioVisualizer.RadioGroup();
  AudioVisualizer.Filters();
