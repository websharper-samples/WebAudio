...
{
 "use strict";
 var Global,Site,Client,Samples,Set,EventTarget,WindowOrWorkerGlobalScope,WebSharper,JavaScript,JS,Sample,Operators,Obj,UI,HtmlModule,attr,List,Doc,SC$1,T,Node,Unchecked,AttrProxy,Pervasives,AttrModule,Array,Client$1,Templates,AudioVisualizer,ChannelSplitter,TimeDomain,Attrs,DomUtility,View,Arrays,Object,Collections,Dictionary,Seq,SC$2,SC$3,SC$4,Snap,DocElemNode,CharacterData,Enumerator,SC$5,HashSet,SC$6,Var,AudioNode,Var$1,Builder,T$1,Elt,Attrs$1,Dyn,DictionaryUtil,Prepare,Slice,KeyCollection,ConcreteVar,AudioHolder,Docs,Updates,Strings,An,Settings,Abbrev,Mailbox,Fresh,ArrayBufferView,Element,HTMLElement,SC$7,Docs$1,RunState,NodeSet,Anims,SC$8,SC$9,HashSetUtil,Queue,SC$10,Concurrency,AppendList,SC$11,Easing,AsyncBody,SC$12,CT,HashSet$1,DynamicAttrNode,Scheduler,CancellationTokenSource,DomNodes,Error,OperationCanceledException,Lazy,SC$13,LazyExtensionsProxy,LazyRecord,IntelliFactory,Runtime,Math,console,$,Date;
 Global=self;
 Site=Global.Site=Global.Site||{};
 Client=Site.Client=Site.Client||{};
 Samples=Site.Samples=Site.Samples||{};
 Set=Samples.Set=Samples.Set||{};
 ...
 Doc=UI.Doc=UI.Doc||{};
 SC$1=Global.StartupCode$WebAudioSamples$Client=Global.StartupCode$WebAudioSamples$Client||{};
 T=List.T=List.T||{};
 Node=Global.Node;
 ...
 AudioVisualizer=Site.AudioVisualizer=Site.AudioVisualizer||{};
 ChannelSplitter=Site.ChannelSplitter=Site.ChannelSplitter||{};
 TimeDomain=Site.TimeDomain=Site.TimeDomain||{};
 ...
 SC$2=Global.StartupCode$WebAudioSamples$AudioVisualizer=Global.StartupCode$WebAudioSamples$AudioVisualizer||{};
 SC$3=Global.StartupCode$WebAudioSamples$ChannelSplitter=Global.StartupCode$WebAudioSamples$ChannelSplitter||{};
 SC$4=Global.StartupCode$WebAudioSamples$TimeDomain=Global.StartupCode$WebAudioSamples$TimeDomain||{};
 ...
 AudioHolder=Site.AudioHolder=Site.AudioHolder||{};
 ...
 SC$9=Global.StartupCode$WebAudioSamples$AudioHolder=Global.StartupCode$WebAudioSamples$AudioHolder||{};
 ...
 SC$11=Global.StartupCode$WebSharper_UI$Abbrev=Global.StartupCode$WebSharper_UI$Abbrev||{};
 ...
 Set=Samples.Set=Runtime.Class({
  Show:function()
  {
   var $this;
   $this=this;
   return Global.jQuery(function()
   {
    var a;
    function select(s,dom)
    {
     Global.jQuery("#sample-navs ul").children("li").removeClass("active");
     Global.jQuery(dom).addClass("active");
     return s.Show();
    }
    self.document;
    a=Doc.Element("ul",[AttrProxy.Create("class","nav nav-pills")],[Doc.Concat(List.mapi(function(i,s)
    {
     return Doc.Element("li",[AttrModule.OnAfterRender(function(self$1)
     {
      if(i===0)
       select(s,self$1);
     }),AttrModule.Handler("click",function(self$1)
     {
      return function()
      {
       return select(s,self$1);
      };
     })],[Doc.Element("a",[AttrProxy.Create("href","#")],[Doc.TextNode(s.Title)])]);
    },$this.$0))]);
    Templates.LoadLocalTemplates("");
    return Doc.RunAppendById("sample-navs",a);
   });
  }
 },null,Set);
 Set.Singleton=function(s)
 {
  return new Set({
   $:0,
   $0:List.ofArray([s])
 ...
 Set.Create=function(ss)
 {
  return new Set({
   $:0,
   $0:List.ofSeq(Seq.delay(function()
   {
 ...
 Sample=Samples.Sample=Runtime.Class({
  Show:function()
  {
   var $this,sMain,sSide,a;
   $this=this;
   sMain=self.document.getElementById("sample-main");
   sSide=self.document.getElementById("sample-side");
   Samples.Clear(sMain);
   Samples.Clear(sSide);
   this.Render(sMain);
   a=Doc.Concat([Doc.Element("div",[AttrModule.OnAfterRender(function(self$1)
   {
    var m,copy;
    m=self.document.getElementById($this.Id);
    Unchecked.Equals(m,null)?void 0:(copy=m.cloneNode(true),copy.attributes.removeNamedItem("id"),self$1.appendChild(copy));
   })],[]),Doc.Element("a",[AttrProxy.Create("class","btn btn-primary btn-lg"),AttrProxy.Create("href","http://github.com/intellifactory/websharper.babylonjs/blob/master/Site/"+this.FileName)],[Doc.TextNode("Source")])]);
   Templates.LoadLocalTemplates("");
   Doc.RunAppendById("sample-side",a);
  }
 },null,Sample);
 Sample.New=function(FileName,Id,Keywords,Render,Title)
 {
  return new Sample({
   FileName:FileName,
   Id:Id,
 ...
 SC$1.$cctor=function()
 {
  ...
  SC$1.All=Set.Create([Set.Singleton(AudioVisualizer.Sample()),Set.Singleton(ChannelSplitter.Sample()),Set.Singleton(TimeDomain.Sample())]);
 };
 ...
 Samples.Clear=function(el)
 {
  while(el.hasChildNodes())
   el.removeChild(el.firstChild);
 };
 Samples.Build=function()
 {
  return Builder.New(null,null,T.Empty,null,null);
 };
 Samples.req=function(name,f)
 {
  return f!=null&&f.$==1?f.$0:Operators.FailWith("Required property not set: "+name);
 };
 Samples.op_PlusPlus=function(a,b)
 {
  return a==null?b:a;
 };
 ...
 AudioVisualizer.Sample=function()
 {
  SC$2.$cctor();
  return SC$2.Sample;
 };
 AudioVisualizer.context=function()
 {
  SC$2.$cctor();
  return SC$2.context;
 };
 AudioVisualizer.filter=function()
 {
  SC$2.$cctor();
  return SC$2.filter;
 };
 AudioVisualizer.Filters=function()
 {
  SC$2.$cctor();
  return SC$2.Filters;
 };
 AudioVisualizer.GenRadioButton=function(filter)
 {
  return Doc.Element("div",[],[Doc.Element("label",[],[Doc.Radio([AttrProxy.Create("name","filters")],filter,AudioVisualizer.rFilter()),Doc.TextNode(Global.String(filter))])]);
 };
 AudioVisualizer.Main=function(elem)
 {
  var a;
  AudioVisualizer.Stop();
  AudioHolder.StopCurrent();
  a=Doc.Concat([AudioVisualizer.Canvas(),Doc.Element("div",[AttrModule.Style("display","inline-block")],AudioVisualizer.RadioGroup())]);
  Templates.LoadLocalTemplates("");
  Doc.Run(elem,a);
  AudioVisualizer.LoadSound("diesirae.mp3",function(buff)
  {
   AudioVisualizer.context().decodeAudioData(buff,function(bff)
   {
    (AudioVisualizer.buffer())[0]=bff;
    AudioVisualizer.Analyser();
    return AudioVisualizer.rFilter().Set("allpass");
   });
  });
 };
 AudioVisualizer.rFilter=function()
 {
  SC$2.$cctor();
  return SC$2.rFilter;
 };
 AudioVisualizer.Stop=function()
 {
  var o;
  o=AudioVisualizer.sourceNode();
  o==null?void 0:o.$0.stop();
 };
 AudioVisualizer.Canvas=function()
 {
  SC$2.$cctor();
  return SC$2.Canvas;
 };
 AudioVisualizer.RadioGroup=function()
 {
  SC$2.$cctor();
  return SC$2.RadioGroup;
 };
 AudioVisualizer.LoadSound=function(url,callback)
 {
  var xhr;
 ...
 AudioVisualizer.buffer=function()
 {
  SC$2.$cctor();
  return SC$2.buffer;
 };
 AudioVisualizer.Analyser=function()
 {
  var analyser,javascriptNode,o,e;
  analyser=AudioVisualizer.context().createAnalyser();
  analyser.smoothingTimeConstant=0.3;
  analyser.fftSize=1024;
  javascriptNode=AudioVisualizer.context().createScriptProcessor(2048,1,1);
  self.sourceNode=javascriptNode;
  javascriptNode.connect(AudioVisualizer.context().destination);
  javascriptNode.onaudioprocess=function()
  {
   var array,ctx,gradient;
   array=new Global.Uint8Array(Operators.toInt(analyser.frequencyBinCount));
   analyser.getByteFrequencyData(array);
   ctx=AudioVisualizer.Canvas().elt.getContext("2d");
   ctx.clearRect(0,0,1000,325);
   gradient=ctx.createLinearGradient(0,0,0,300);
   AudioVisualizer.MkGradient(gradient);
   ctx.fillStyle=gradient;
   return AudioVisualizer.DrawSpectrum(ctx,array);
  };
  AudioVisualizer.filter().type="allpass";
  AudioVisualizer.filter().connect(analyser);
  AudioVisualizer.set_sourceNode({
   $:1,
   $0:AudioVisualizer.context().createBufferSource()
  });
  analyser.connect(javascriptNode);
  AudioVisualizer.filter().connect(AudioVisualizer.context().destination);
  o=AudioVisualizer.sourceNode();
  o==null?void 0:(e=o.$0,AudioHolder.SetCurrent(e),e.connect(AudioVisualizer.filter()),e.buffer=(AudioVisualizer.buffer())[0],e.loop=true,e.start());
 };
 AudioVisualizer.sourceNode=function()
 {
  SC$2.$cctor();
  return SC$2.sourceNode;
 };
 AudioVisualizer.MkGradient=function(gradient)
 {
 ...
 AudioVisualizer.DrawSpectrum=function(ctx,array)
 {
  List.iteri(function(i,a)
  {
   return ctx.fillRect(i*5,325-a,3,325);
  },AudioVisualizer.ToList(array));
 };
 AudioVisualizer.set_sourceNode=function($1)
 {
  SC$2.$cctor();
  SC$2.sourceNode=$1;
 };
 AudioVisualizer.ToList=function(from)
 {
  var n,res,n$1;
  n=from.length-1;
  res=T.Empty;
 ...
 ChannelSplitter.Sample=function()
 {
  SC$3.$cctor();
  return SC$3.Sample;
 };
 ChannelSplitter.volume=function()
 {
  SC$3.$cctor();
  return SC$3.volume;
 };
 ChannelSplitter.Main=function(elem)
 {
  var a;
  ChannelSplitter.Stop();
  AudioHolder.StopCurrent();
  a=Doc.Concat([ChannelSplitter.Canvas(),ChannelSplitter.VolumeControl()]);
  Templates.LoadLocalTemplates("");
  Doc.Run(elem,a);
  ChannelSplitter.LoadSound("diesirae.mp3",function(buff)
  {
   ChannelSplitter.context().decodeAudioData(buff,function(bff)
   {
    (ChannelSplitter.buffer())[0]=bff;
    return ChannelSplitter.Analyser();
   });
  });
 };
 ChannelSplitter.Stop=function()
 {
  var o;
  o=ChannelSplitter.sourceNode();
  o==null?void 0:o.$0.stop();
 };
 ChannelSplitter.Canvas=function()
 {
  SC$3.$cctor();
  return SC$3.Canvas;
 };
 ChannelSplitter.VolumeControl=function()
 {
  SC$3.$cctor();
  return SC$3.VolumeControl;
 };
 ChannelSplitter.LoadSound=function(url,callback)
 {
  var xhr;
  xhr=new Global.XMLHttpRequest();
  xhr.open("GET",url,true);
 ...
 ChannelSplitter.buffer=function()
 {
  SC$3.$cctor();
  return SC$3.buffer;
 };
 ChannelSplitter.Analyser=function()
 {
  var analyser1,analyser2,gainNode,javascriptNode,splitter,o,e;
  analyser1=ChannelSplitter.context().createAnalyser();
  analyser1.smoothingTimeConstant=0.3;
  analyser1.fftSize=1024;
  analyser2=ChannelSplitter.context().createAnalyser();
  analyser2.smoothingTimeConstant=0;
  analyser2.fftSize=1024;
  gainNode=ChannelSplitter.context().createGain();
  javascriptNode=ChannelSplitter.context().createScriptProcessor(2048,1,1);
  self.sourceNode=javascriptNode;
  javascriptNode.connect(ChannelSplitter.context().destination);
  javascriptNode.onaudioprocess=function()
  {
   var array,average1,array2,average2,ctx,gradient;
   array=new Global.Uint8Array(Operators.toInt(analyser1.frequencyBinCount));
   analyser1.getByteFrequencyData(array);
   average1=Seq.average(List.map(Global.id,ChannelSplitter.ToList(array)));
   array2=new Global.Uint8Array(Operators.toInt(analyser2.frequencyBinCount));
   analyser2.getByteFrequencyData(array2);
   average2=Seq.average(List.map(Global.id,ChannelSplitter.ToList(array2)));
   ctx=ChannelSplitter.Canvas().elt.getContext("2d");
   ctx.clearRect(0,0,60,130);
   gradient=ctx.createLinearGradient(0,0,0,130);
   ChannelSplitter.MkGradient(gradient);
   ctx.fillStyle=gradient;
   ctx.fillRect(0,130-average1,25,130);
   ctx.fillRect(30,130-average2,25,130);
   gainNode.gain.value=(ChannelSplitter.volume())[0];
  };
  splitter=ChannelSplitter.context().createChannelSplitter();
  ChannelSplitter.set_sourceNode({
   $:1,
   $0:ChannelSplitter.context().createBufferSource()
  });
  gainNode.connect(splitter);
  splitter.connect(analyser1,0,0);
  splitter.connect(analyser2,1,0);
  analyser1.connect(javascriptNode);
  gainNode.connect(ChannelSplitter.context().destination);
  o=ChannelSplitter.sourceNode();
  o==null?void 0:(e=o.$0,AudioHolder.SetCurrent(e),e.connect(gainNode),e.loop=true,e.buffer=(ChannelSplitter.buffer())[0],e.start());
 };
 ChannelSplitter.context=function()
 {
  SC$3.$cctor();
  return SC$3.context;
 };
 ChannelSplitter.sourceNode=function()
 {
  SC$3.$cctor();
  return SC$3.sourceNode;
 };
 ChannelSplitter.MkGradient=function(gradient)
 {
  gradient.addColorStop(1,"#000000");
  gradient.addColorStop(0.75,"#ff0000");
  gradient.addColorStop(0.25,"#ffff00");
 ...
 ChannelSplitter.ToList=function(from)
 {
  var n,res,n$1;
  n=from.length-1;
  res=T.Empty;
 ...
 ChannelSplitter.set_sourceNode=function($1)
 {
  SC$3.$cctor();
  SC$3.sourceNode=$1;
 };
 TimeDomain.Sample=function()
 {
  SC$4.$cctor();
  return SC$4.Sample;
 };
 TimeDomain.context=function()
 {
  SC$4.$cctor();
  return SC$4.context;
 };
 TimeDomain.filter=function()
 {
  SC$4.$cctor();
  return SC$4.filter;
 };
 TimeDomain.Filters=function()
 {
  SC$4.$cctor();
  return SC$4.Filters;
 };
 TimeDomain.GenRadioButton=function(filter)
 {
  return Doc.Element("div",[],[Doc.Element("label",[],[Doc.Radio([AttrProxy.Create("name","filters")],filter,TimeDomain.rFilter()),Doc.TextNode(Global.String(filter))])]);
 };
 TimeDomain.Main=function(elem)
 {
  var a;
  TimeDomain.Stop();
  AudioHolder.StopCurrent();
  a=Doc.Concat([TimeDomain.CanvasEl(),Doc.Element("div",[AttrModule.Style("display","inline-block")],TimeDomain.RadioGroup())]);
  Templates.LoadLocalTemplates("");
  Doc.Run(elem,a);
  TimeDomain.LoadSound("diesirae.mp3",function(buff)
  {
   TimeDomain.context().decodeAudioData(buff,function(bff)
   {
    (TimeDomain.buffer())[0]=bff;
    TimeDomain.Analyser();
    return TimeDomain.rFilter().Set("allpass");
   });
  });
 };
 TimeDomain.rFilter=function()
 {
  SC$4.$cctor();
  return SC$4.rFilter;
 };
 TimeDomain.Stop=function()
 {
  var o;
  o=TimeDomain.sourceNode();
  o==null?void 0:o.$0.stop();
 };
 TimeDomain.CanvasEl=function()
 {
  SC$4.$cctor();
  return SC$4.CanvasEl;
 };
 TimeDomain.RadioGroup=function()
 {
  SC$4.$cctor();
  return SC$4.RadioGroup;
 };
 TimeDomain.LoadSound=function(url,callback)
 {
  var xhr;
  xhr=new Global.XMLHttpRequest();
  xhr.open("GET",url,true);
 ...
 TimeDomain.buffer=function()
 {
  SC$4.$cctor();
  return SC$4.buffer;
 };
 TimeDomain.Analyser=function()
 {
  var analyser,javascriptNode,o,e;
  analyser=TimeDomain.context().createAnalyser();
  analyser.smoothingTimeConstant=0.3;
  analyser.fftSize=1024;
  javascriptNode=TimeDomain.context().createScriptProcessor(2048,1,1);
  self.sourceNode=javascriptNode;
  javascriptNode.connect(TimeDomain.context().destination);
  javascriptNode.onaudioprocess=function()
  {
   var array;
   array=new Global.Uint8Array(Operators.toInt(analyser.frequencyBinCount));
   analyser.getByteTimeDomainData(array);
   return TimeDomain.DrawTimeDomain(TimeDomain.CanvasEl().elt.getContext("2d"),array);
  };
  TimeDomain.filter().type="allpass";
  TimeDomain.filter().connect(analyser);
  TimeDomain.set_sourceNode({
   $:1,
   $0:TimeDomain.context().createBufferSource()
  });
  analyser.connect(javascriptNode);
  TimeDomain.filter().connect(TimeDomain.context().destination);
  o=TimeDomain.sourceNode();
  o==null?void 0:(e=o.$0,AudioHolder.SetCurrent(e),e.connect(TimeDomain.filter()),e.buffer=(TimeDomain.buffer())[0],e.loop=true,e.start());
 };
 TimeDomain.sourceNode=function()
 {
  SC$4.$cctor();
  return SC$4.sourceNode;
 };
 TimeDomain.DrawTimeDomain=function(ctx,array)
 {
  var c,width,height;
  c=$(TimeDomain.CanvasEl().elt);
  width=c.width();
  height=c.height();
  ...
  List.iteri(function(i,a)
  {
   ...
   return ctx.fillRect(i,height-height*(a/256)-1,1,1);
  },TimeDomain.ToList(array));
 };
 TimeDomain.set_sourceNode=function($1)
 {
  SC$4.$cctor();
  SC$4.sourceNode=$1;
 };
 TimeDomain.ToList=function(from)
 {
  var n,res,n$1;
  n=from.length-1;
  res=T.Empty;
 ...
 SC$2.$cctor=function()
 {
  ...
  SC$2.context=new(Global.AudioContext||Global.webkitAudioContext)();
  SC$2.sourceNode=null;
  SC$2.filter=AudioVisualizer.context().createBiquadFilter();
  SC$2.buffer=[null];
  SC$2.Canvas=Doc.Element("canvas",[AttrProxy.Create("width","1000"),AttrProxy.Create("height","325")],[]);
  SC$2.rFilter=Var$1.Lens(Var$1.Create$1("allpass"),Global.id,function(a,x)
  {
   AudioVisualizer.filter().type=x;
   return x;
  });
  SC$2.Filters=List.ofArray(["allpass","bandpass","highpass","highshelf","lowpass","lowshelf","notch","peaking"]);
  SC$2.RadioGroup=List.map(AudioVisualizer.GenRadioButton,AudioVisualizer.Filters());
  SC$2.Sample=Samples.Build().Id("AudioVisualizer").FileName("AudioVisualizer.fs").Keywords(List.ofArray(["webaudio","visualizer","frequency","spectrum"])).Render(function(e)
  {
   AudioVisualizer.Main(e);
  }).Create();
 };
 SC$3.$cctor=function()
 {
  SC$3.$cctor=Global.ignore;
  SC$3.context=new(Global.AudioContext||Global.webkitAudioContext)();
  SC$3.sourceNode=null;
  SC$3.buffer=[null];
  SC$3.Canvas=Doc.Element("canvas",[AttrProxy.Create("width","60"),AttrProxy.Create("height","130"),AttrProxy.Create("style","display: block;")],[]);
  SC$3.volume=[1];
  SC$3.VolumeControl=Doc.Element("input",[AttrProxy.Create("type","range"),AttrProxy.Create("min","1"),AttrProxy.Create("max","100"),AttrProxy.Create("value","100"),AttrProxy.Create("style","width: 60px"),AttrModule.Handler("input",function(el)
  {
   return function()
   {
    var fraction;
    fraction=$(el).val()/$(el).prop("max");
    (ChannelSplitter.volume())[0]=fraction*fraction;
   };
  })],[]);
  SC$3.Sample=Samples.Build().Id("ChannelSplitter").FileName("ChannelSplitter.fs").Keywords(List.ofArray(["webaudio","vizualiser","channels","volume control"])).Render(function(e)
  {
   ChannelSplitter.Main(e);
  }).Create();
 };
 SC$4.$cctor=function()
 {
  SC$4.$cctor=Global.ignore;
  SC$4.context=new(Global.AudioContext||Global.webkitAudioContext)();
  SC$4.sourceNode=null;
  SC$4.filter=TimeDomain.context().createBiquadFilter();
  SC$4.buffer=[null];
  SC$4.CanvasEl=Doc.Element("canvas",[AttrProxy.Create("width","512"),AttrProxy.Create("height","256"),AttrProxy.Create("style","background-color: black")],[]);
  SC$4.rFilter=Var$1.Lens(Var$1.Create$1("allpass"),Global.id,function(a,x)
  {
   TimeDomain.filter().type=x;
   return x;
  });
  SC$4.Filters=List.ofArray(["allpass","bandpass","highpass","highshelf","lowpass","lowshelf","notch","peaking"]);
  SC$4.RadioGroup=List.map(TimeDomain.GenRadioButton,TimeDomain.Filters());
  SC$4.Sample=Samples.Build().Id("TimeDomainVisualizer").FileName("TimeDomain.fs").Keywords(List.ofArray(["webaudio","timedomain","visualizer","audio"])).Render(function(e)
  {
   TimeDomain.Main(e);
  }).Create();
 };
 ...
 Builder=Samples.Builder=Runtime.Class({
  Create:function()
  {
   var id,title,x;
   id=Samples.req("Id",Samples.op_PlusPlus(this.BId,this.BTitle));
   title=(x=Samples.op_PlusPlus(this.BTitle,this.BId),x==null?"Sample":x.$0);
   return Sample.New(Samples.req("FileName",this.BFileName),id,this.BKeywords,Samples.req("Render",this.BRender),title);
  },
  Render:function(x)
  {
   this.BRender={
 ...
 Builder.New=function(BFileName,BId,BKeywords,BRender,BTitle)
 {
  return new Builder({
   BFileName:BFileName,
   BId:BId,
 ...
 AudioHolder.StopCurrent=function()
 {
  var o;
  o=AudioHolder.currentPlaying();
  o==null?void 0:o.$0.stop();
 };
 AudioHolder.currentPlaying=function()
 {
  SC$9.$cctor();
  return SC$9.currentPlaying;
 };
 AudioHolder.SetCurrent=function(src)
 {
  AudioHolder.set_currentPlaying({
   $:1,
   $0:src
  });
 };
 AudioHolder.set_currentPlaying=function($1)
 {
  SC$9.$cctor();
  SC$9.currentPlaying=$1;
 };
